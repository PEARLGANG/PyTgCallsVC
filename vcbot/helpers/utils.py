...
def raw_converter(source, vid, audio, log_file='ffmpeg.log'):
    ...
    cmd = ["ffmpeg", "-y", "-hide_banner", "-loglevel", "error", "-i", source, "-f", "s16le", "-ac", "1", "-ar", "48000", audio, "-f", "rawvideo", '-r', '24', '-pix_fmt', 'yuv420p', '-vf', 'scale=864:-1', vid]
...
async def transcode(file_path: str, delete=True):
    ...
    cmd = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", file_path, "-f", "s16le", "-ac", "1", "-ar", "48000", audio_f, "-f", "rawvideo", '-r', '24', '-pix_fmt', 'yuv420p', '-vf', 'scale=864:-1', video_f]
    proc = await asyncio.create_subprocess_exec(
        *cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
